...
class AotterPlayer {
    ...
    _constructHiddenIframe() { // to fix CORS issue. 
    ...
    _cbAjaxOk(data = {}) {
        ...
        if(!data.success) {
            this.cbAdFailed ? this.cbAdFailed(this.domSelector) : '';
    ...
    _loadImg(data) { // ref: https://github.com/aotter/trek-web-sdk-doc/blob/master/docs/customize.md
        ...
        // if not find at HTML , apply JS config width/height ; suggest use JS to config only ; no HTML config
        ...
        // Trigger GPU render when using CSS 3D
    ...
    _loadVid(data) {
        ...
        // load script as google's docs ; this is better way
        // since when user reload the browser; this method is beter than register at HTML header
        // register in HTML header, based on my POC, the video will not able to loaded;
        // On the other hand, you below method, it has no issue when browser reload.
        // I have tried : https://github.com/aotter/trek-web-sdk-doc/blob/master/docs/nativeVideoAd.md
        // to load the script, but not as good as I think.
        let tag = document.createElement('script');
        tag.src = "https://www.youtube.com/iframe_api";
        let firstScriptTag = document.getElementsByTagName('script')[0];
        firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);
        // google docs use onYouTubeIframeAPIReady callback event; but it is not good in our lib;
        // since the onYouTubeIframeAPIReady need to be gobal regsitered
        // the below method is from stack overflow
    ...
    _loadYoutube() {
     ...
     _onPlayerReady(event) {
       ...
       this.cbAdLoaded ? this.cbAdLoaded(this.domSelector) : 'no cb found';
     }
...
AotterPlayer.loaded = true; // like google's YT.loaded for checking this file is full loaded in memory.
window.AotterPlayer = AotterPlayer;
